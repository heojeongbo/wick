...
// WICK_VERSION, which is why one that is not recognized is reported rather
// than refused.
const EnvPrefix = "WICK_"
...
// after the path to it, so `dsn` of `db` is read from WICK_DB_DSN.
//
